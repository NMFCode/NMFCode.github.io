...
The synchronization directions are reflected by the enumeration [SynchronizationDirection](api/NMF.Synchronizations.SynchronizationDirection.yml).
...
In this synchronization direction, the synchronization engine will not perform any changes but rather list inconsistencies found during synchronization. These detected differences are then reported as instances of _[IInconsistency](api/NMF.Synchronizations.Inconsistencies.IInconsistency.yml)_ in the property **Inconsistencies** of the [synchronization context](api/NMF.Synchronizations.ISynchronizationContext.yml). The interface for inconsistencies has methods to fix the inconsistency on either side.
